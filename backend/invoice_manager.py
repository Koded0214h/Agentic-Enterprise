...
def create_invoice(
    ...
    invoice = {
        ...
        'items': items, # List of dicts: [{'description': 'item desc', 'quantity': 1, 'unit_price': 100.0}]
...
def generate_invoice_content(invoice, template_path='invoice_template.txt'):
    ...
    for item in invoice['items']:
        items_str += f"- {item['description']} (Qty: {item['quantity']}) @ {invoice['currency']}{item['unit_price']:.2f} = {invoice['currency']}{item['quantity'] * item['unit_price']:.2f}\n"
